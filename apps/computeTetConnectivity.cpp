...
/* given a tet-mesh in umesh form, computes the connectiviy of this
   mesh, and dumps it to given output file name */ 
...
namespace umesh {
  ...
  {
    if (error != "") std::cout << "Error: " << error << std::endl << std::endl;

    std::cout << "usage: umeshComputeTetConnectivity in.umesh -o out.tetconn" << std::endl;
    exit(error != "");
  }
